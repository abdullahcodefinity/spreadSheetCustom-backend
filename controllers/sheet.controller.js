...
export const createSheet = async (req, res) => {
 try {
  ...
  if (userRole !== "SuperAdmin") {
   return res
    .status(403)
    .json({ error: true, message: "Only SuperAdmin can create sheets" });
  }
  ...
  if (!name || !Array.isArray(columns)) {
   return res
    .status(400)
    .json({ error: "Name and columns array are required" });
  }
...
export const getSheet = async (req, res) => {
 try {
  const { id } = req.params;
  const { page = 1, limit = 50 } = req.query; // Default to page 1 with 50 items per page
  const userId = req.user.userId;
  const userRole = req.user.role;
  ...
  if (isNaN(pageSize) || pageSize < 1 || pageSize > 500) {
   return res
    .status(400)
    .json({ error: "Invalid limit (must be between 1 and 500)" });
  }
  ...
  // First get the sheet with metadata but without rows (for better performance)
  const sheetMetadata = await prisma.sheet.findFirst({
   where: {
    id: Number(id),
   },
   include: {
  ...
  // Get total rows count for pagination info
  const totalRows = await prisma.sheetData.count({
   where: {
    spreadsheetId: Number(id),
   },
  });
  ...
  // Combine metadata with paginated data
  const sheet = {
   ...sheetMetadata,
   sheetData: sheetData,
  };
  ...
  // Add the frozenColumns property and pagination info to the response
  const responseSheet = {
   ...sheet,
   frozenColumns,
   pagination: {
...
export const updateSheetColumns = async (req, res) => {
 try {
  const { id } = req.params;
  const { newColumnName, insertAtIndex, deleteAtIndex, updateAtIndex } =
   req.body;
  const userId = req.user.userId;
  const userRole = req.user.role;
  ...
  if (userRole !== "SuperAdmin") {
   ...
   if (typeof insertAtIndex === "number") {
    const hasAddPermission = userSheet.permissions.some(
     (p) => p.type === "addColumn"
    );
    if (!hasAddPermission) {
     return res
   ...
   if (typeof deleteAtIndex === "number") {
    const hasDeletePermission = userSheet.permissions.some(
     (p) => p.type === "deleteColumn"
    );
  ...
  if (typeof insertAtIndex === "number" && !newColumnName) {
   return res
    .status(400)
    .json({ message: "Missing newColumnName for column insertion" });
  }

  if (typeof updateAtIndex === "number" && !newColumnName) {
   return res
    .status(400)
    .json({ message: "Missing newColumnName for column update" });
  }

  if (typeof insertAtIndex === "number" && insertAtIndex < 0) {
   return res
    .status(400)
    .json({ message: "Invalid insertAtIndex: must be non-negative" });
  }
  ...
  if (
   ...
   insertAtIndex > updatedColumns.length
  ) {
   return res
    .status(400)
    .json({ message: "Invalid insertAtIndex: exceeds column count" });
  }

  // Only allow one operation at a time to avoid complexity
  const operationCount = [insertAtIndex, deleteAtIndex, updateAtIndex].filter(
  ...
  if (operationCount > 1) {
   return res.status(400).json({
    message:
     "Cannot perform multiple operations simultaneously. Please perform one at a time.",
   });
  }
  ...
  if (operationCount === 0) {
   return res.status(400).json({
    message:
     "No operation specified. Provide insertAtIndex, deleteAtIndex, or updateAtIndex.",
   });
  }
 ...
 try {
  ...
  if (userRole !== "SuperAdmin") {
   ...
   if (!hasColumnPermission) {
    return res
     .status(403)
     .json({ message: "You do not have permission to move columns" });
   }
  }
  ...
  if (
   sourceIndex < 0 ||
   sourceIndex >= updatedColumns.length ||
   targetIndex < 0 ||
 ...
 try {
  ...
  if (action === "attach") {
   if (!fileType) {
    return res
     .status(400)
     .json({ message: "fileType is required for attach action" });
   }
  ...
  res.json({
   ...
   message:
    action === "attach"
     ? "File type attached successfully"
...
export const attachCalendar = async (req, res) => {
 try {
  ...
  res.json({
   ...
   message:
    action === "attach"
     ? "Calendar attached successfully"
...
export const attachEmail = async (req, res) => {
 try {
  ...
  res.json({
   ...
   message:
    action === "attach"
     ? "Email functionality attached successfully"
     : "Email functionality removed successfully",
  });
 } catch (error) {
...
export const attachFormula = async (req, res) => {
 try {
  const { id: sheetId } = req.params;
  const { columnName, formula, action = "attach" } = req.body; // action can be 'attach' or 'remove'
  const userId = req.user.userId;
  const userRole = req.user.role;
  ...
  // Validate input for attach action
  if (action === "attach" && !formula) {
   return res.status(400).json({ message: "Formula is required for attach action" });
  }
  ...
  if (action === "attach") {
   // Attach or update formula to the column
   columnsMeta[columnName] = {
    ...columnsMeta[columnName],
    formula: formula,
   };
  } else if (action === "remove") {
   // Remove formula from the column
   if (columnsMeta[columnName]) {
    delete columnsMeta[columnName].formula;
    // If the column meta is now empty, remove the column meta entirely
    if (Object.keys(columnsMeta[columnName]).length === 0) {
     delete columnsMeta[columnName];
    }
  ...
  res.json({
   ...
   message:
    action === "attach"
     ? "Formula attached successfully"
     : "Formula removed successfully",
  });
 } catch (error) {
  console.error("Error managing formula:", error);
  res.status(500).json({ message: "Internal server error" });
 }
};
...
export const attachDropdown = async (req, res) => {
 try {
  ...
  res
   .status(500)
   .json({ error: true, message: "Error attaching dropdown to column" });
 }
};
...
export const removeDropdown = async (req, res) => {
 try {
  ...
  res
   .status(500)
   .json({ error: true, message: "Error removing dropdown from column" });
 }
};
...
export const shareSheet = async (req, res) => {
 try {
  ...
  if (currentUserRole !== "SuperAdmin") {
   return res
    .status(403)
    .json({ error: true, message: "Only SuperAdmin can manage sheet access." });
  }
  ...
  const sheet = frozenColumns
   ? await prisma.sheet.findUnique({
      where: { id: Number(id) },
      select: { columns: true },
     })
   : null;
...
export const updateSheetPermissions = async (req, res) => {
 try {
  ...
  if (currentUserRole !== "SuperAdmin") {
   return res
    .status(403)
    .json({ error: true, message: "Only SuperAdmin can update permissions." });
  }
  ...
  if (permissions && !Array.isArray(permissions)) {
   return res
    .status(400)
    .json({ error: true, message: "permissions must be an array." });
  }
  ...
  if (!userSheet) {
   return res
    .status(404)
    .json({ error: true, message: "User does not have access to this sheet." });
  }
...
export const removeUserFromSheet = async (req, res) => {
 try {
  ...
  if (currentUserRole !== "SuperAdmin") {
   return res
    .status(403)
    .json({ error: true, message: "Only SuperAdmin can remove user access." });
  }
  ...
  res
   .status(500)
   .json({ error: true, message: "Error removing user from sheet." });
 }
};
